...
Catalogue.hasMany(Processes, { foreignKey: "id_catalogue", as: "processes" });
Processes.belongsTo(Catalogue, { foreignKey: "id_catalogue" });
...
app.post("/createTxtFile", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: `Archivo ${filePath} creado exitosamente` });
  } catch (error) {
    console.error("Error al crear el archivo:", error);
    res
      .status(500)
      .json({ error: "Error interno del servidor al crear el archivo" });
  }
});
...
app.get("/readTxtFile/:fileName", (req, res) => {
  ...
  fs.readFile(filePath, "utf8", (err, data) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ error: "Error interno del servidor al leer el archivo" });
    } else {
